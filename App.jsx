...
import { Plus, Trash2, Edit2, LogOut, CheckCircle, ChevronRight, MessageCircle, User, Check, X, Timer } from 'lucide-react';
    ...
    <div className="relative min-h-screen bg-[#FDFCF0]">
      ...
      {!isCampaignStarted && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 pointer-events-none">
          <div className="max-w-[290px] w-full bg-[#064E3B]/100 border border-white/20 p-6 rounded-[32px] shadow-2xl backdrop-blur-[0px] text-center text-white pointer-events-auto">
            <Timer className="w-8 h-8 text-[#D4AF37] mx-auto mb-3 animate-pulse" />
            <h2 className="text-xl font-bold mb-1">സജ്ജമാകൂ!</h2>
            <p className="text-[11px] text-emerald-50 mb-4 opacity-80">കാമ്പയിൻ തുടങ്ങാൻ ഇനി...</p>
            ...
            <div className="grid grid-cols-4 gap-1.5 mb-5">
              {[
                ...
                <div key={t.label} className="bg-black/20 p-1.5 rounded-xl border border-white/5">
                  <div className="text-lg font-black text-[#D4AF37] leading-none">{String(t.val).padStart(2, '0')}</div>
                  <div className="text-[7px] uppercase tracking-tighter opacity-40 mt-1">{t.label}</div>
                </div>
              ))}
            </div>

            <div className="space-y-2">
              <div className="bg-[#D4AF37]/100 text-[#fff] py-2 rounded-xl text-sm font-black shadow-md">
                 JANUARY 21, 2026
              </div>
              
              <a 
                href="https://wa.me/917907997569?text=ഞാൻ ശഅബാൻ സ്വലാത്ത് കാമ്പയിനിൽ പങ്കെടുക്കാൻ ആഗ്രഹിക്കുന്നു. തുടങ്ങിയാൽ എന്നെ അറിയിക്കണം.. 👍"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 w-full py-2.5 bg-white/10 hover:bg-white/20 border border-white/10 rounded-xl text-[10px] font-bold transition-all active:scale-95"
              >
                <MessageCircle className="w-3.5 h-3.5 text-emerald-400" />
                തുടങ്ങുമ്പോൾ അറിയിക്കുക
              </a>
            </div>
          </div>
        </div>
      ...
      {/* --- MAIN UI WRAPPER --- */}
      <div className={!isCampaignStarted ? "pointer-events-none select-none" : ""}>
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 backdrop-blur-sm animate-in fade-in">
